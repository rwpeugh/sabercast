...
- **Sabercast's player_matcher retrieves actual 2025 free-agent signings at 3.1× the random-chance rate** (precision@10 = 41.9% vs 13.3% baseline, n=43, p < 0.0001). When a team had a flagged top-3 gap and signed an FA at that position, the actual signer appears in the recommended top-10 about 42% of the time. The deployed Gap Filler's candidate list is meaningful.
  - *Note on what this measures:* the claim is about the **retrieval layer** (`find_matches`'s top-10 by semantic similarity). Downstream stages of the deployed orchestrator — incumbent-aware composite re-ranking (Entry 25), three-tier bucketing into bargain / at-budget / premium (Entry 29) — re-order and surface a subset of those 10. They do not change *whether* the actual signing is in the 10; only *how it's presented to the user*. The 3.1× lift therefore measures the underlying retrieval quality and is invariant to the orchestrator's display logic. Re-running `eval/precision_at_k.py` after the Entry 25-29 architectural changes produces bit-for-bit identical numbers, confirming this separation cleanly.
